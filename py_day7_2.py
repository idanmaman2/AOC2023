...
def create_key(line):
    ...
    if prince_count:=counted.get('J',0): 
        counted.pop('J')
    if sorted_counted:=sorted(counted.values()): 
        sorted_counted[-1]+=prince_count
    else : 
        sorted_counted=(5,)
    return "".join(sorted(map(str,collections.Counter(sorted_counted).items()),reverse=True))
